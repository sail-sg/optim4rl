...
def time_info(exp, file_name="log.txt", runs=1, nbins=10, max_line_length=10000):
    ...
    if len(time_list) > 0:
        ...
        print(f"{exp} max time: {np.max(time_list):.4f} minutes")
        print(f"{exp} mean time: {np.mean(time_list):.4f} minutes")
        print(f"{exp} min time: {np.min(time_list):.4f} minutes")
...
if __name__ == "__main__":
    for agent_config in os.listdir("./configs/"):
        ...
        print(
            f"The number of total combinations in {agent_config}:",
